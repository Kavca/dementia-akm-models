...
Here is where we build the Concept_CM model to define and agree on the Concepts in the problem space with the user.  

The goal is to scope the model-work, start exploring which Information, Roles, Tasks and Views are used. Its normal to make several Modelview with examples to understand the Domain. 

It will result in an agreed Concept model suggesting the Typenames to be used in the next step, which is the Typedefinition model used to generate the metamodel.  

<details>
<summary>Design Thinking (Generated by ChatGPT)</summary>
